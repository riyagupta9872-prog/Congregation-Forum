...
async function openReminderModal() {
  ...
  // Look up each devotee's gender so reminders address them correctly —
  // male → "prabhuji", female (or unknown) → "mataji".
  const genderByName = {};
  try {
    const devotees = await DevoteeCache.all();
    devotees.forEach(d => { if (d.name) genderByName[d.name] = d.gender; });
  } catch (_) {}
  const honorific = name => genderByName[name] === 'Male' ? 'prabhuji' : 'mataji';
...
async function loadCallingHistory() {
  ...
  try {
    ...
    if (_chCache && _chCache.key === chKey && Date.now() - _chCache.ts < _CH_TTL) {
      ...
      el.innerHTML = '<div class="loading"><i class="fas fa-spinner"></i> Loading…</div>';
      ({ weeks, devotees, submMap } = await DB.getCallingHistoryGrid(teamFilter, callerFilter, deptFilter));
...
async function loadSaidComingTab() {
  ...
  const dept = (typeof getFilterDept === 'function') ? getFilterDept() : '';
  const _scFilter = (raw) => {
    let list = team ? raw.filter(d => (d.team_name || d.teamName) === team) : raw;
    if (!team && dept && typeof matchesDept === 'function') {
      list = list.filter(d => matchesDept({ teamName: d.team_name || d.teamName, department: d.department, gender: d.gender }, dept));
    }
    return list;
  };
  ...
  try {
    ...
    const list = _scFilter(result?.list || []);
    _renderCorrelationTab(el, list, '😕 Said Coming — Didn\'t Come', '#dc2626', 'Confirmed on call but absent on session', true);
  } catch (e) {
    el.innerHTML = '<div class="empty-state"><i class="fas fa-exclamation-circle"></i><p>Failed to load</p></div>';
